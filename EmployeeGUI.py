...
class EmployeeGUI:      
    ...
    def setup_buttons(self):
        ...
        for i, text in enumerate(button_texts):
            ...
            button.grid(row=8 + i, column=0, columnspan=2, padx=10, pady=5, sticky="ew")  # Add padding and make buttons fill entire width

    def add_employee(self):
        employee_id = self.entries["Employee ID:"].get()
        # Check if an employee with the same ID already exists
        try:
            existing_employees = FileHandler.load_data("employees_data.pkl")
        ...
        FileHandler.save_data(existing_employees, "employees_data.pkl")
